...
2. Create a directory called `manifests` which will include all our Kubernetes configurations.
...
3. Gateway Service Code - In `src` folder create another service folder: `gateway`
...
- Tag the Docker Image (Image ID is the text attached to sha256:{...})
```cmd
docker tag <image_id> mashilo/gateway:latest
```
- Push the image to the remote repository
```cmd
docker push mashilo/gateway:latest
```
- Pull the image
```cmd
docker pull mashilo/gateway:latest
```

5. Create the k8s `manifests` directory
...
---
7. Create the `converter` directory. [Converter Service Deployment]
